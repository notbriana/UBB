...
void testIteratorRemove() {
	cout << "Test Iterator Remove" << endl;
...

	{
		...
		MapIterator it = m.iterator();
		assert(it.valid());
		TElem removed = it.remove();
		assert(removed.first == 10 && removed.second == 100);
		assert(m.size() == 0);
		assert(!it.valid());
	}

	{
		...
		assert(it.valid());
		TElem removed = it.remove();
		assert(removed.second == 10 || removed.second == 20 || removed.second == 30);
		assert(m.size() == 2);
		assert(it.valid()); 
	}

	{
		...
		while (it.valid()) {
			TElem removed = it.remove();
			assert(removed.first >= 0 && removed.first < 5);
			removedCount++;
		}
		assert(removedCount == 5);
		assert(m.size() == 0);
	}
