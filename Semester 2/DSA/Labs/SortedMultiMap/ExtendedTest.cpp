...
bool rel(TKey a, TKey b) { return a < b; }
...
void test_addIfNotPresent() {
	SortedMultiMap map1(rel);
	SortedMultiMap map2(rel);
	...
	int added = map1.addIfNotPresent(map2);
	assert(added == 2);

	assert(map1.search(1).size() == 1);
	assert(map1.search(2).size() == 1);
	assert(map1.search(3).size() == 1);
	assert(map1.search(4).size() == 1);

	int added2 = map1.addIfNotPresent(map2);
	assert(added2 == 0);

	cout << "test_addIfNotPresent passed.\n";
}
