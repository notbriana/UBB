...
public class Repository implements IRepository {
    ...
    public void logPrgStateExec(PrgState state) throws CollectionException {
        try (PrintWriter logFile = new PrintWriter(new BufferedWriter(new FileWriter(logFilePath, true)))) {
            ...
            var barrierTable = state.barrierTable().getContent();
            for (var entry : barrierTable.entrySet()) {
                int index = entry.getKey();
                int limit = entry.getValue().getKey();
                var waitingList = entry.getValue().getValue();
                logFile.println("   " + index + " --> Limit: " + limit + ", Waiting: " + waitingList);
            }
